 ...
 */
 ...
 #include "obj-manager-test.h" // test.cpp 상단에 반드시 추가
 ...
 int main()
 {
     ...
     auto manager = std::make_unique<ManagerAdaptor>(*connection, ObjectPath{"/com/example/ble"});
     while (true)
     {
         printCountDown("Creating PlanetAdaptor in ", 5);
         auto earth = std::make_unique<PlanetAdaptor>(*connection, ObjectPath{"/com/example/ble/Planet1/Earth"}, "Earth", 7'874'965'825);
         printCountDown("Creating PlanetAdaptor in ", 5);
         auto trantor = std::make_unique<PlanetAdaptor>(*connection, ObjectPath{"/com/example/ble/Planet1/Trantor"}, "Trantor", 40'000'000'000);
         printCountDown("Creating PlanetAdaptor in ", 5);
         auto laconia = std::make_unique<PlanetAdaptor>(*connection, ObjectPath{"/com/example/ble/Planet1/Laconia"}, "Laconia", 231'721);
